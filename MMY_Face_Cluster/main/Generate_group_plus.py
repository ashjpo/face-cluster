...
class Generate_group_plus:
    def __init__(self,generated_group_list,faceobj_list,max_face_th,max_up_limit_percentage,if_compare_generated=False,if_debug=False):
        """
                传入faceobj对象列表
        """
        self.if_debug=if_debug
        #是否在加入新faceobj时候对已经归档的组扫描
        self.if_compare_generated=if_compare_generated
    ...
    def get_group(self):
        return self.group_list,self.generated_group_list
    ...
    def archive_group(self):
        ...
        for i in range(len(self.no_grouped_faceobj)):
            ...
            for group in self.generated_group_list:
                ...
                if if_add==True:
                    ...
                    add_group_faceobj.append(faceobj)
                    
                    if self.if_compare_generated:
                        #计算组内
                        remove_faceobj_list=group.group_self_compare(self.max_up_limit_percentage)
                        for remove_fobj in remove_faceobj_list:
                            if remove_fobj in add_group_faceobj:
                                add_group_faceobj.remove(remove_fobj)
                                
                    break
        
        for fobj in add_group_faceobj:
            #print(fobj,len(self.no_grouped_faceobj))
        ...
        #self.group_list=self.group_list+self.generated_group_list
